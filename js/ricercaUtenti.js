  ...
  if (post.length > 0) {
    ...
    for(let i=0; i < post.length; i++){
      ...
      <li class="list-group-item border-0">
        <a href="profilo.php?user=${post[i]["iduser"]}">${post[i]["nome"]} ${post[i]["cognome"]} ${post[i]["linkImmagine"]}</a>
      </li>`;
    }
    result += `</ul>`;
  } else {
    result += `<ul class="list-group position-absolute dropdown-menu" style="width:40%; background-color: transparent; display: none;"></ul>`;
  }
  return result;
}
